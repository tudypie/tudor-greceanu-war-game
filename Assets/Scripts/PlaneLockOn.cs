...
public class PlaneLockOn : MonoBehaviour
{
    ...
    // Mouse-driven free reticle, in screen pixels (origin bottom-left, y up).
    Vector2 _reticleScreen;
    // Smoothed crosshair actually drawn: follows the reticle, or snaps to a
    // locked target when the soft aim assist has one.
    ...
    // Reticle position relative to screen center, each axis in [-1, 1] at the
    // edge of its travel. PlaneCameraFollow leans the chase cam by this.
    ...
    void MoveReticle(float dt)
    {
        ...
        // While RMB free-look is active the mouse drives the camera, so the
        // reticle is frozen (no move, no recenter) to keep aim stable.
        ...
        // The bounds rectangle constrains the whole lock-on box, not just the
        // crosshair point: inset the center's travel by half the box so the
        // box edges stop at the brackets instead of poking past them.
        var s = UiScale;
    ...
    // The fixed rectangle the reticle is clamped within, in screen pixels
    // (origin bottom-left). Mirrors the clamp math in MoveReticle.
    ...
    void PushAimToShooter()
    {
        ...
        // RMB free-look (no lock): the camera is panning, so freeze the aim
        // at its last direction instead of slewing it through the moving ray.
        ...
        // Free aim: fire from the muzzle toward the world point under the
        // reticle. Bullets converge there, so putting the reticle on a target
        // is a hit regardless of where the plane's nose points.
    ...
    // L-shaped marks at the four corners of a rect, leaving the sides open.
