...
public class PlaneFlightModel : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        var deltaPitch = (InvertPitch ? -PitchInput : PitchInput) * PitchIncreaseSpeed * agility * dt;
        var bank = _transform.right.y;

        float deltaRoll;
        if (Mathf.Approximately(RollInput, 0f))
        {
            deltaRoll = -bank * RollAutoLevelSpeed * agility * dt;
        }
        else
        {
            deltaRoll = -RollInput * RollIncreaseSpeed * agility * dt;
        }

        var deltaYaw = (YawInput * YawSpeed - bank * BankTurnSpeed) * agility * dt;
