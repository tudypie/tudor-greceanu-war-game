...
// Shared faction-agnostic fighter base: FSM (Patrol/Engage/Disengage), targeting,
// retaliation, crowd cap, firing. Flight safety is an overlay, not a state.
// Faction doctrine lives in the EnemyAI / AllyAI subclasses via the virtual seams.
...
public abstract class PlaneAIController : MonoBehaviour
{
    ...
    // Shared count of AIs pursuing each friendly plane, kept in sync via
    // SetTarget/OnDestroy so a slot never leaks.
    static readonly System.Collections.Generic.Dictionary<PlaneHealth, int>
        s_attackerCount = new();
    PlaneHealth _countedTarget;

    // When shot, lock onto the attacker: overrides player bias, range loss and
    // disengages (terrain safety still wins); each hit extends the deadline.
    ...
    // Lagged-position ring buffer: steering lag plus target-velocity estimate.
    ...
    // Runs from PlaneHealth.TakeDamage, outside FixedUpdate: sets the target now;
    // the state transition follows in the next UpdateState.
    ...
    void FixedUpdate()
    {
        ...
        // Before UpdateState so transitions and the steering overlay see this tick's threat.
        UpdateGroundThreat();

        UpdateState();

        // As terrain threat rises, suppress lead and avoidance and blend the
        // aim toward the recoverable climb-out point.
        ...
        // GCAS-off fallback: soft upward bias toward the floor.
        if (!Stats.GcaEnabled)
            aimPoint += ComputeAltitudeSoftBias(anticipatedFloor);

        // Ceiling mirror, only when the cap is above the floor so a ridge
        // poking above it is never clamped back down.
        ...
        // GCAS overlay: at high threat, full nose-up wings-level pull (bypasses PitchGain).
        ...
        // Layer 3: below the actual floor, abandon everything and climb out
        // wings-level guns-cold until a hysteresis margin above it is clawed back.
    ...
    // Clearance above ground, never below PatrolMinWorldY (also the no-terrain case).
    ...
    // Highest working floor along a..b, so a route clears a hill mid-leg, not just at its endpoint.
    ...
    // Highest working floor out to the look-ahead point, so a ridge lifts the
    // floor (and the clamped aim) before we reach it.
    ...
    // True travel velocity so a dive is sensed down its real path; falls back
    // to the nose when near-stationary.
    ...
    // Projects the trajectory forward into a [0,1] threat plus a recoverable
    // climb-out point. Optional pull-up sim relaxes false positives over
    // rising terrain; a 3-ray fan covers the curve a banking plane flies.
    void UpdateGroundThreat()
    {
        ...
        var raw = Mathf.Max(depthScore, timeScore);

        // Arm fast for safety, release slow to kill chatter.
        var tau = raw > _threat
    ...
    // Idle/loiter aim point. AllyAI overrides it for formation; the terrain/
    // ceiling/boundary/GCAS overlays run after this so an override stays safe.
    ...
    // Lagged position plus a velocity lead so the AI cuts the corner instead
    // of tail-chasing forever.
    ...
    void UpdateState()
    {
        // Retaliation override: hunt whoever shot us now, cutting patrol/disengage short.
        ...
        switch (_state)
        {
            case AIState.Patrol:
                if (_target != null)
                {
                    // On a failed roll, peel off and wander so the swarm scatters.
            ...
            case AIState.Engage:
                ...
                // Small per-refresh chance to break contact and wander; rolled
                // on the refresh cadence so the mean attention span holds.
    ...
    // Window/Overshoot keep the target and its slot then re-engage; LoseInterest
    // drops the target (freeing the slot) and wanders longer.
    void EnterDisengage(DisengageReason reason)
    {
        ...
        // Extend along the heading, biased away from the target, so the AI
        // separates and the player gets a clean window.
    ...
    // Pick a fresh waypoint once the current one is reached or times out;
    // shared by Patrol and the LoseInterest wander.
    ...
    // Selection cost for `ph` (lower = preferred). Base is nearest-hostile;
    // subclasses bias it (EnemyAI de-prioritises the human, AllyAI defends it).
    protected virtual float ScoreCandidate(
        PlaneHealth ph, float distSqFromSelf, float playerScoreMul)
        => distSqFromSelf;

    // Selection only; commit/peel-off is the FSM's call.
    void RefreshTarget(bool force)
    {
        // Locked on whoever shot us; a killed/expired attacker falls through.
        if (RetaliationActive)
        {
        ...
        // Stay distracted for the whole LoseInterest window (entry already dropped the target).
        ...
        // PlayerTargetBias squared, fed to ScoreCandidate; the AcquireRange
        // gate below still uses the true distance.
        ...
        // No target: adopt one inside AcquireRange (true distance). Commit-or-wander is Patrol's call.
        if (_target == null)
        {
        ...
        // Already chasing: only switch to a clearly-better hostile so it won't
        // ditch an ally for a slightly-closer player.
    ...
    // Wipe the shared count per session so it survives Enter Play Mode with
    // domain reload disabled (no ghost attackers).
    ...
    // Moves this AI's one slot from its old target to the new one (null =
    // release). The only place s_attackerCount is mutated.
    ...
    // At cap? Counts only other AIs (our own slot is discounted) so we never
    // evict ourselves off a target we hold.
    ...
    void UpdateFiring()
    {
        ...
        if (_state == AIState.Engage && _target != null)
        {
            ...
            if (dist > 0.0001f && inRange)
            {
                ...
                {
                    // The gun is hitscan, so aim at the target's current
                    // position; GunLeadTime only covers the solve-to-shot sub-step.
                    ...
                    // Gate on the undegraded solution; the residual spray
                    ...
                    // Residual spray scaled by range and off-tail aspect,
                    // jittered fast so a burst spreads instead of walking on.
    ...
    // Mirrors PlaneShooter's hitscan: only worth firing if the first body the
    // ray meets is the target (else terrain/another plane masks it).
    ...
    void PickNewPatrolWaypoint()
    {
        ...
        // Clamp inside the map BEFORE sampling the floor, so the route uses the
        // corrected XZ (an edge spawn would otherwise throw legs into the turn-back).
        var boundary = MapBoundary.Instance;
        if (boundary != null)
    ...
    void OnDrawGizmos()
    {
        ...
        // Ground track coloured by live threat, with the climb-out marker.
    ...
    // Firing envelope: FireConeDeg half-angle, FireRange long, FireMinDistance dead zone.
