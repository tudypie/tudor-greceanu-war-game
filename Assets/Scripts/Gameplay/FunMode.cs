...
// Sandbox cheat panel: drop on any scene object. Stat tweaks go on runtime
// clones so the committed ScriptableObjects and AI planes are untouched.
public class FunMode : MonoBehaviour
{
    ...
    // No global pitch knob exists, so drive every live AudioSource directly;
    // rescanned periodically to catch transient one-shot/explosion sources.
    ...
    // Mini-nuke at the bullet impact point; raycast mirrors PlaneShooter.Fire
    // so the blast lands where the round does. Rate-limited.
    void OnShot()
    {
    ...
    // Cosmetic only: spawns rigidbody cubes off the sampled terrain surface;
    // the Unity Terrain is never modified.
    ...
    // One erupted cube: counts against the global cap, shrinks out over its
    // final half-second.
