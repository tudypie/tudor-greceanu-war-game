...
// Mission-1 protect target. Thin objective facade over a sibling PlaneHealth
// (the single HP store): enemy guns already damage it through the normal
// PlaneShooter -> GetComponentInParent<PlaneHealth> path. Mission fails when
...
public class Airfield : MonoBehaviour
{
    ...
    // Direct entry for a striker (or anything); the bullet path reaches the
    // same HP store via PlaneHealth.TakeDamage.
