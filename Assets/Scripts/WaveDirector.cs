...
// Mission-1 enemy spawner: an ordered list of waves, each a set of compass
// spawn groups. The first N planes of a group get an AirfieldStrikeRun (they
// bomb the airfield); the rest keep the prefab's EnemyAI (they hunt the
// player/allies). Mirrors PlaneSpawner's terrain-safe placement but does not
// use or modify the shared PlaneSpawner.
public class WaveDirector : MonoBehaviour
{
    ...
    // Final wave cleared (or timed out). A destroyed airfield is a loss, not a
    // win — that failure path is owned by Airfield's own state.
    ...
    // Centered line with a cheap drop shadow so it stays legible over sky or
    // terrain. Caller restores skin/color state.
