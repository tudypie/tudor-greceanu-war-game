...
// Non-spatial (2D) player feedback. Put this ONLY on the player plane —
// these are cockpit/HUD cues, not world sounds, so they ignore distance.
//
//   * Enemy kill  - PlaneShooter.Killed (this plane's shot finished a hostile)
//   * Got hit     - PlaneHealth.Damaged, rate-limited so sustained fire
//                   doesn't machine-gun the sample
//   * Overheat    - PlaneShooter.OverheatStarted (gun hit its heat limit)
//   * Lock-on     - PlaneLockOn.LockAcquired (target acquired)
//   * Radio beeps - a random clip from a collection at a random interval
