...
// Mission-2 (Vest) win condition: clear the sky. Watches the OneShot enemy
// PlaneSpawners; the mission is won once every one has emitted its full
// squadron and none remain alive, with the player still alive. The loss case
// (player death) is left to the existing PlayerDeathRestart.
//
// The Vest analogue of Airfield: a thin scene-placed objective facade that
// owns no gameplay state of its own.
